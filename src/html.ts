...
import { Node, Edge } from './types'; // Asumiendo que defines estos tipos en un fichero types.ts
...
const generateLegendHtml = (): string => {
  return Object.entries(config.colors).map(([key, color]) => {
    ...
    const label = config.labels[key] || key.charAt(0).toUpperCase() + key.slice(1);
 ...
 */
const generateHtmlContent = (nodes: Node[], edges: Edge[], legendHtml: string): string => `
  <!DOCTYPE html>
  <html>
    <head>
 ...
 */
export function generateHtml(files: string[], dependencies: Map<string, string[]>, selectedDirectory: string): void {
